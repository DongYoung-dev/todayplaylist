...
export class PlaylistsService {
    ...
    async viewPlaylist(playlistId: string, forModify: boolean) {
        ...
        if (forModify === false) {
            ...
            const userRecentCount = await this.recentRepository.count({ where: { userId: userId } })
            ...
            //viewCount 1 증가
            await this.playlistRepository.update({ playlistId }, { viewCount: () => "viewCount + 1" })
        }

        //해당 플레이리스트 정보 반환
        const playlist = await this.playlistRepository.findOne({ where: { playlistId: playlistId } })
        const likeCount = await this.likeRepository.count({ where: { playlistId: playlist.playlistId } })
        const isLiked = await this.likeRepository.findOne({ where: { userId, playlistId: playlist.playlistId } })
    ...
    async searchPlaylist(q: string, page: number, size: number) {
        ...
        const parsedPlaylists = await Promise.all(playlists.map(async (playlist) => {
            const likeCount = await this.likeRepository.count({ where: { playlistId: playlist.playlistId } })
            const isLiked = await this.likeRepository.findOne({ where: { userId, playlistId: playlist.playlistId } })
    ...
    async saveSong(videoId: string, title: string) {
        const existVideo = await this.videoRepository.findOne({ where: { videoId: videoId } })
        ...
        if (existVideo) {
            ...
            const key = process.env.YOUTUBE_KEY
            const contentDetails = await lastValueFrom(this.http.get(
            ...
            if (matches && matches.length >= 4) {
                ...
                const formattedTime = hours ? `${hours}:${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}` : `${minutes.toString().padStart(2, '0')}:${seconds.toString().padStart(2, '0')}`;
    ...
    async searchLikedPlaylist(q: string, page: number, size: number) {
        ...
        const filteredPlaylists = await Promise.all(allLikedPlaylists.map(async (playlist) => {
            const thisPlaylist = await this.playlistRepository.findOne({ where: { playlistId: playlist.playlistId } });
            const likeCount = await this.likeRepository.count({ where: { playlistId: playlist.playlistId } });
            const isLiked = await this.likeRepository.findOne({ where: { userId, playlistId: playlist.playlistId } });

            if (q && (thisPlaylist.title.includes(q) || thisPlaylist.hashtag.includes(q))) {
                return {
                    ...thisPlaylist,
                    likeCount,
    ...
    async getRegisteredPlaylist(page: number, size: number) {
        ...
        const playlists = allRegisteredPlaylists.slice((page - 1) * size, page * size)
    ...
    async getRecentPlaylist() {
        ...
        const parsedPlaylists = await Promise.all(allRecentPlaylists.map(async (playlist) => {
            const thisPlaylist = await this.playlistRepository.findOne({ where: { playlistId: playlist.playlistId } })
            const likeCount = await this.likeRepository.count({ where: { playlistId: playlist.playlistId } })
            const isLiked = await this.likeRepository.findOne({ where: { userId, playlistId: playlist.playlistId } })
